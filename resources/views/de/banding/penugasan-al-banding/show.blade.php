...
@section('title', 'Detail Penugasan AL Banding Banding - ' . $pengajuan->nomor_pengajuan)
...
<div class="container-fluid py-3">
    ...
    <nav aria-label="breadcrumb" class="mb-3">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="{{ route('de.banding.penugasan-al-banding') }}">Penugasan AL Banding</a></li>
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-4 gap-2">
        <div>
            <h5 class="mb-1">
                <i class="bi bi-building"></i> Detail Penugasan AL Banding
            </h5>
            <small class="text-muted">{{ $pengajuan->nomor_pengajuan }}</small>
        </div>
        <a href="{{ route('de.banding.penugasan-al-banding') }}" class="btn btn-secondary">
    ...
    <div class="row">
        ...
        <div class="col-lg-8 mb-4">
            ...
            <!-- Status Alert -->
            @if($log?->status_to === \App\Models\PengajuanAkreditasi::STATUS_ASESOR_AL_BANDING_ASSIGNED)
            <div class="alert alert-success alert-permanent">
                <i class="bi bi-person-check"></i>
                <strong>Penugasan Asesor AL Banding</strong><br>
                Penugasan asesor AL banding telah dilakukan. Mohon memastikan asesor banding telah menyetujui penawaran asesmen lapangan
            </div>
            @elseif($log?->status_to === \App\Models\PengajuanAkreditasi::STATUS_AL_BANDING_IN_PROGRESS)
            <div class="alert alert-info alert-permanent">
                <i class="bi bi-clock-history"></i>
                <strong>Asesmen Lapangan Banding Berlangsung</strong><br>
                Asesor AL banding telah ditugaskan dan proses asesmen lapangan sedang berlangsung
            </div>
            @elseif($log?->status_to === \App\Models\PengajuanAkreditasi::STATUS_AL_BANDING_SELESAI)
            <div class="alert alert-info alert-permanent">
                <i class="bi bi-clock-history"></i>
                <strong>Asesmen Lapangan Banding Telah Selesai</strong><br>
                Asesor AL banding telah ditugaskan dan proses asesmen lapangan telah berlangsung
            </div>
            @endif
                    ...
                    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center rounded gap-3">
                        <div class="d-flex align-items-center">
                            ...
                            <div>
                                ...
                                <br>
                                <span class="badge bg-success">Surat Tugas Asesor AL Banding</span>
                        ...
                        <div class="btn-group-vertical">
                            ...
                            <a href="{{ route('de.banding.penugasan-al-banding.download-surat-tugas', [$pengajuan->id, 'surat_tugas_asesor_al_banding']) }}" class="btn btn-success mb-2" target="_blank">
            ...
            <div class="card mb-4">
                <div class="card-header bg-primary text-white">
                    <h5 class="mb-0">
                        <i class="bi bi-person-plus"></i> Tugaskan Asesor AL Banding
                    </h5>
                </div>
                <div class="card-body">
                    <form id="assignForm" onsubmit="assignAsesor(event, {{ $pengajuan->id }})">
                        <div class="row g-3">
                            <div class="col-md-6">
                                <label class="form-label fw-bold">Pilih Asesor Banding: <span class="text-danger">*</span></label>
                                <select id="userId" class="form-select" required data-no-select2>
                                    <option value="">-- Pilih Asesor Banding --</option>
                                </select>
                            </div>
                            <div class="col-md-6">
                            ...
                            <div class="col-12">
                                <button type="submit" class="btn btn-success">
                                    <i class="bi bi-plus-circle"></i> Tugaskan Asesor Banding
                                </button>
                            </div>
                        </div>
                    </form>
            ...
            <div class="card mb-4">
                <div class="card-header bg-white d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">Daftar Penugasan AL Banding</h5>
                ...
                <tr>
                    <td colspan="4" class="text-center py-4 text-muted">
                        Belum ada asesor banding yang ditugaskan
                    </td>
                </tr>
                @endforelse
                </tbody>
    ...
    <!-- Informasi Penugasan AL Banding -->
    <div class="card">
        <div class="card-header bg-secondary text-white">
            <h5 class="mb-0">
                <i class="bi bi-info-circle"></i> Informasi Penugasan AL Banding
            </h5>
        </div>
        <div class="card-body">
            <table class="table table-borderless">
                <tr>
                    <th>Tanggal Penugasan Asesor AL Banding</th>
                    <td>
                        : {{ $pengajuan->tanggal_penugasan_asesor_al_banding
                                    ? $pengajuan->tanggal_penugasan_asesor_al_banding->locale('id')->translatedFormat('d M Y H:i')
                                    : '-' }}
                    </td>
                </tr>
                @if($pengajuan->asesmen?->asesmenLapanganBanding)
                <tr>
                    <th>Tanggal Mulai AL Banding</th>
                    <td>
                        : {{ $pengajuan->asesmen->asesmenLapanganBanding->tanggal_mulai
                                    ? \Carbon\Carbon::parse($pengajuan->asesmen->asesmenLapanganBanding->tanggal_mulai)->locale('id')->translatedFormat('d M Y')
                                    : '-' }}
                    </td>
                </tr>
                <tr>
                    <th>Estimasi Tanggal Selesai AL Banding</th>
                    <td>
                        : {{ $pengajuan->asesmen->asesmenLapanganBanding->tanggal_selesai
                                    ? \Carbon\Carbon::parse($pengajuan->asesmen->asesmenLapanganBanding->tanggal_selesai)->locale('id')->translatedFormat('d M Y')
                                    : '-' }}
                    </td>
                </tr>
                ...
                @endif
                <tr>
                    <th>Status Penugasan Asesor AL Banding</th>
                    <td>: {!! $pengajuan->getCustomBadgeLastStatus('penugasan_asesor_al_banding', 'de', 'label_long_for') !!}</td>
                </tr>
            </table>
        </div>
    </div>
...
<div class="col-lg-4">
                ...
                <div class="timeline-item mb-3">
                    <div class="d-flex">
                        <div class="flex-shrink-0">
                            ...
                            \App\Models\PengajuanAkreditasi::STATUS_ASESOR_AL_BANDING_ASSIGNED => 'text-success',
                            \App\Models\PengajuanAkreditasi::STATUS_AL_BANDING_IN_PROGRESS => 'text-info',
    ...
    <div class="card mt-4">
        <div class="card-header bg-info text-white d-flex justify-content-between align-items-center">
            <h5 class="mb-0">
                <i class="bi bi-calendar-range"></i> Jadwal Visitasi AL Banding
            </h5>
        ...
        <div class="card-body">
            @php $al = $pengajuan->asesmen->asesmenLapanganBanding; @endphp
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-primary text-white">
                <h5 class="modal-title">
                    <i class="bi bi-upload"></i> Upload Surat Tugas Asesor AL Banding
                </h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <form id="formUploadSuratTugas" method="POST" action="{{ route('de.banding.penugasan-al-banding.upload-surat-tugas', [$pengajuan->id, 'surat_tugas_asesor_al_banding']) }}" enctype="multipart/form-data">
                @csrf
                <div class="modal-body">
                    <div class="mb-3">
...
<script>
    $(document).ready(function () {
        $('#userId').select2({
            theme: 'bootstrap-5',
            width: '100%',
            placeholder: '-- Pilih Asesor Banding --',
            allowClear: true,
            ajax: {
    ...
    async function assignAsesor(event, pengajuanId) {
        ...
        try {
            const response = await fetch(`/de/banding/penugasan-al-banding/${pengajuanId}/assign-asesor`, {
                method: 'POST'
                , headers: {
    ...
    async function removeAsesor(pengajuanId, userId, userName) {
        const result = await Swal.fire({
            ...
            , html: `Hapus <strong>${userName}</strong> dari penugasan AL Banding?`
            , icon: 'warning'
        ...
        try {
            const response = await fetch(`/de/banding/penugasan-al-banding/${pengajuanId}/remove-asesor/${userId}`, {
                method: 'DELETE'
                , headers: {
