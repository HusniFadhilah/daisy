...
@section('title', 'Detail Pelaksanaan AL Banding - ' . $pengajuan->nomor_pengajuan)
...
<div class="container-fluid py-3">
    ...
    <nav aria-label="breadcrumb" class="mb-3">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="{{ route('de.banding.pelaksanaan-al-banding') }}">Pelaksanaan AL Banding</a></li>
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-4 gap-2">
        <div>
            <h5 class="mb-1">
                <i class="bi bi-geo-alt"></i> Detail Pelaksanaan AL Banding
            </h5>
            <small class="text-muted">{{ $pengajuan->nomor_pengajuan }}</small>
        </div>
        <a href="{{ route('de.banding.pelaksanaan-al-banding') }}" class="btn btn-secondary">
    ...
    <div class="row">
        ...
        <div class="col-lg-8 mb-4">
            ...
            <!-- Status Alert -->
            @if($log?->status_to === \App\Models\PengajuanAkreditasi::STATUS_AL_BANDING_IN_PROGRESS)
            <div class="alert alert-info alert-permanent">
                <i class="bi bi-clock-history"></i>
                <strong>Asesmen Lapangan Banding Berlangsung</strong><br>
                Proses asesmen lapangan banding sedang berlangsung. Pantau progress surveillance asesor secara berkala
            </div>
            ...
            <div class="alert alert-success alert-permanent">
                ...
                Asesmen lapangan banding telah selesai dilaksanakan. <br>Menunggu validator untuk membuat rekap dan laporan
            </div>
            @elseif($log?->status_to === \App\Models\PengajuanAkreditasi::STATUS_AL_BANDING_DILAPORKAN)
            <div class="alert alert-success alert-permanent">
                <i class="bi bi-check-circle"></i>
                <strong>Asesmen Lapangan Banding Selesai</strong><br>
...
<div class="card mb-4">
    ...
    <div class="card-body">
        ...
        <div class="alert alert-warning alert-permanent mb-0">
            ...
            <p class="mb-0 mt-2">
                Validator diperlukan untuk membuat rekap laporan AL banding
            </p>
        </div>
        @else
        <div class="table-responsive">
...
<div class="card mb-4">
    ...
    <div class="card-body">
        @php
        $beritaAcaraBandingList = $pengajuan->asesmen?->beritaAcaraALBanding ?? collect([]);
        @endphp

        @if($beritaAcaraBandingList->count() > 0)
        <div class="alert alert-info alert-permanent mb-3">
            <i class="bi bi-info-circle"></i>
            Berikut adalah berita acara pelaksanaan asesmen lapangan banding
        </div>

        @foreach($beritaAcaraBandingList as $index => $beritaAcaraBanding)
            ...
            <div class="card-body">
                <div class="d-flex align-items-start">
                    ...
                    <div class="flex-grow-1">
                        <h6 class="mb-1">
                            <strong>{{ $beritaAcaraBanding->title }}</strong>
                        </h6>
                        <small class="text-muted">
                            Diupload: {{ $beritaAcaraBanding->uploaded_at ? $beritaAcaraBanding->uploaded_at->locale('id')->translatedFormat('d M Y H:i') : '-' }}
                        </small>
                        <br>
                        <small class="text-muted">
                            Oleh: {{ $beritaAcaraBanding->uploader->name ?? '-' }}
                        </small>
                    </div>
        ...
        <div class="text-center py-4">
            ...
            <small class="text-muted">Berita acara akan diupload oleh asesor banding setelah visitasi selesai</small>
        </div>
        @endif
    </div>
</div>
...
<div class="card mb-4">
    ...
    <div class="card-body">
        ...
        <div class="alert alert-info alert-permanent mb-3">
            <i class="bi bi-info-circle"></i>
            Berikut adalah laporan hasil asesmen lapangan banding yang telah diupload oleh asesor
        </div>
...
<!-- Informasi Pelaksanaan AL Banding -->
<div class="card">
    <div class="card-header bg-secondary text-white">
        <h5 class="mb-0">
            <i class="bi bi-info-circle"></i> Informasi Pelaksanaan AL Banding
        </h5>
    </div>
    <div class="card-body">
        <table class="table table-borderless">
            ...
            <tr>
                <th>Tanggal Selesai AL Banding</th>
                <td>
            ...
            @endif
            <tr>
                <th>Status Pelaksanaan AL Banding</th>
                <td>: {!! $pengajuan->getCustomBadgeLastStatus('pelaksanaan_al_banding', 'de', 'label_long_for') !!}</td>
            </tr>
        </table>
    </div>
</div>
...
<div class="col-lg-4">
                ...
                <div class="timeline-item mb-3">
                    <div class="d-flex">
                        <div class="flex-shrink-0">
                            ...
                            \App\Models\PengajuanAkreditasi::STATUS_AL_BANDING_SELESAI => 'text-success',
    ...
    <div class="card my-4">
        ...
        <div class="card-body">
            @php $alBanding = $pengajuan->asesmen->asesmenLapanganBanding; @endphp
            <table class="table table-sm table-borderless mb-0">
                @if($alBanding->tanggal_mulai)
                <tr>
                    <th class="text-muted" width="45%">Tanggal Mulai</th>
                    <td>: <strong>{{ \Carbon\Carbon::parse($alBanding->tanggal_mulai)->locale('id')->translatedFormat('d M Y') }}</strong></td>
                </tr>
                @endif
                @if($alBanding->tanggal_selesai)
                <tr>
                    <th class="text-muted">Tanggal Selesai</th>
                    <td>: <strong>{{ \Carbon\Carbon::parse($alBanding->tanggal_selesai)->locale('id')->translatedFormat('d M Y') }}</strong></td>
                </tr>
                @endif
                @if($alBanding->tanggal_mulai && $alBanding->tanggal_selesai)
                <tr>
                    <th class="text-muted">Durasi</th>
                    <td>:
                        @php
                        $start = \Carbon\Carbon::parse($alBanding->tanggal_mulai);
                        $end = \Carbon\Carbon::parse($alBanding->tanggal_selesai);
                ...
                @endif
                @if($alBanding->lokasi_visitasi)
                <tr>
                    <th class="text-muted">Lokasi</th>
                    <td>:
                        <i class="bi bi-geo-alt-fill text-danger"></i>
                        {{ $alBanding->lokasi_visitasi }}
                    </td>
                </tr>
                @endif
            </table>
    ...
    <div class="card mb-4">
        <div class="card-header bg-success text-white">
            <h5 class="mb-0">
                <i class="bi bi-file-earmark-check"></i> Status Berita Acara Banding
            </h5>
        </div>
        <div class="card-body">
            <div class="d-flex align-items-center mb-2">
                ...
                <div>
                    <strong>Berita Acara Banding Tersedia</strong>
                    <br>
                    <small class="text-muted">
        ...
        <div class="modal-content">
            ...
            <form id="formAssignValidator" onsubmit="submitAssignValidator(event)">
                <div class="modal-body">
                    <div class="alert alert-info alert-permanent">
                        <i class="bi bi-info-circle"></i>
                        Validator akan bertanggung jawab untuk membuat rekap laporan AL banding
                    </div>
...
<script>
    ...
    async function submitAssignValidator(event) {
        ...
        try {
            const response = await fetch(`/de/banding/pelaksanaan-al-banding/${currentPengajuanId}/assign-validator`, {
                method: 'POST'
                , headers: {
    ...
    async function removeValidator(pengajuanId, userId, userName) {
        ...
        try {
            const response = await fetch(`/de/banding/pelaksanaan-al-banding/${pengajuanId}/remove-validator/${userId}`, {
                method: 'DELETE'
                , headers: {
                    'Content-Type': 'application/json'
                    , 'X-CSRF-TOKEN': csrfToken
