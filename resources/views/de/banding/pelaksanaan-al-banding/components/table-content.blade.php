<div class="card">
    <div class="card-header bg-light d-flex justify-content-between align-items-center">
        <h6 class="mb-0">
            <i class="bi bi-table"></i> Daftar Pelaksanaan AL Banding
        </h6>
    ...
    <div class="card-body p-0">
            ...
            <table class="table table-hover align-middle mb-0">
                <thead class="table-light">
                    <tr>
                        ...
                        <th width="17%">Status Pelaksanaan AL Banding</th>
                ...
                <tbody>
                    ...
                    if ($aur->role_selected->name === 'asesor_banding') {
                    $asesorCount++;
                    ...
                    }elseif ($pengajuan->status === \App\Models\PengajuanAkreditasi::STATUS_AL_BANDING_SELESAI) {
                    // Check if has validator
                    if ($validatorCount > 0) {
                    $statusBadge = ['class' => 'warning', 'icon' => 'person-check', 'text' => 'Validator Ditugaskan'];
                    } else {
                    $statusBadge = ['class' => 'warning', 'icon' => 'exclamation-triangle', 'text' => 'Perlu Validator'];
                    }
                    } elseif ($pengajuan->status === \App\Models\PengajuanAkreditasi::STATUS_AL_BANDING_IN_PROGRESS) {
                    $statusBadge = ['class' => 'primary', 'icon' => 'hourglass-split', 'text' => 'Visitasi Berlangsung'];
                    } elseif ($pengajuan->status === \App\Models\PengajuanAkreditasi::STATUS_ASESOR_AL_BANDING_ASSIGNED) {
                    $statusBadge = ['class' => 'secondary', 'icon' => 'clock', 'text' => 'Asesor Banding Ditugaskan'];
                    } else {
                    ...
                    <tr>
                        ...
                        <td>
                            @if($asesorCount > 0)
                            <div>
                                <i class="bi bi-people"></i> <strong>{{ $asesorCount }}</strong> asesor banding
                            </div>
                            @else
                        ...
                        <td>
                            {!! $pengajuan->getCustomBadgeLastStatus('pelaksanaan_al_banding','de','label_short_for') !!}
                            {{-- <span class="badge bg-{{ $statusBadge['class'] }}">
                            <i class="bi bi-{{ $statusBadge['icon'] }}"></i> {{ $statusBadge['text'] }}
                            </span> --}}
                        </td>
                        <td>
                            @if($pengajuan->tanggal_al_banding_selesai)
                            <small>{{ $pengajuan->tanggal_al_banding_selesai->locale('id')->translatedFormat('d M Y') }}</small>
                            <br>
                            <small class="text-muted">
                                {{ $pengajuan->tanggal_al_banding_selesai->diffForHumans() }}
                            </small>
                            @else
                        ...
                        <td class="text-center">
                            <a href="{{ route('de.banding.pelaksanaan-al-banding.show', $pengajuan->id) }}" class="btn btn-sm btn-primary" title="Lihat Detail">
                                <i class="bi bi-eye"></i>
                            </a>
                        </td>
                    </tr>
        ...
        <div class="text-center py-5">
            ...
            <p class="text-muted mt-3 mb-0">Tidak ada AL banding yang sedang berlangsung</p>
        </div>
        @endif
    </div>
