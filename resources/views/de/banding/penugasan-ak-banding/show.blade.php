{{-- resources\views\de\banding\penugasan-ak-banding\show.blade.php --}}
@extends('layouts.template.app')

@section('title', 'Detail Penugasan AK Banding')
...
<div class="container-fluid py-3">
    ...
    <nav aria-label="breadcrumb" class="mb-3">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="{{ route('de.banding.penugasan-ak-banding') }}">Penugasan AK Banding</a></li>
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-4 gap-2">
        <div>
            <h5 class="mb-1">
                <i class="bi bi-person-check"></i> Detail Penugasan AK Banding
            </h5>
            <small class="text-muted mb-0">{{ $pengajuan->nomor_pengajuan }}</small>
        </div>
        <div class="d-flex gap-2">
            @if(in_array($pengajuan->status, [
            \App\Models\PengajuanAkreditasi::STATUS_PEMBAYARAN_BANDING_DIVERIFIKASI,\App\Models\PengajuanAkreditasi::STATUS_BANDING_DILAKSANAKAN,
            ]) && !$pengajuan->asesmen?->asesmenKecukupanBanding)
            <button class="btn btn-success" onclick="showMarkReadyModal({{ $pengajuan->id }})">
                <i class="bi bi-check-circle"></i> Tetapkan Siap untuk AK Banding
            </button>
            @endif
            <a href="{{ route('de.banding.penugasan-ak-banding') }}" class="btn btn-secondary">
    ...
    <div class="row">
        ...
        <div class="col-lg-8 mb-4">
            @if($pengajuan->asesmen?->asesmenKecukupanBanding)
            @php
            $allowed = [
            ...
            @endphp
            <!-- Status Alert -->
            @if($log?->status_to === \App\Models\PengajuanAkreditasi::STATUS_ASESOR_AK_BANDING_ASSIGNED)
            <div class="alert alert-success alert-permanent">
                <i class="bi bi-person-check"></i>
                <strong>Penugasan Asesor AK Banding</strong><br>
                Penugasan asesor AK banding telah dilakukan<br>
                Mohon memastikan asesor dan validator telah menyetujui penawaran asesmen
            </div>
            @elseif($log?->status_to === \App\Models\PengajuanAkreditasi::STATUS_AK_BANDING_IN_PROGRESS)
            <div class="alert alert-success alert-permanent">
                <i class="bi bi-person-check"></i>
                <strong>Penugasan Asesor AK Banding</strong><br>
                Asesor AK banding telah ditugaskan untuk melakukan penilaian
            </div>
            @elseif($log?->status_to === \App\Models\PengajuanAkreditasi::STATUS_AK_BANDING_SELESAI)
            <div class="alert alert-success alert-permanent">
                <i class="bi bi-person-check"></i>
                <strong>Penugasan Asesor AK Banding</strong><br>
                Asesor AK banding telah ditugaskan dan telah menyelesaikan penilaian
            </div>
            @endif
            ...
            <div class="card mb-4">
                <div class="card-header bg-primary text-white">
                    <h5 class="mb-0">
                        <i class="bi bi-person-plus"></i> Tugaskan Asesor Banding / Validator
                    </h5>
                </div>
                <div class="card-body">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-8" id="validatorOptionsContainer" style="display: none;">
                                ...
                                <div class="alert alert-info alert-permanent mb-3">
                                    <div class="d-flex align-items-center justify-content-between">
                                        <div>
                                            ...
                                            <strong>Validator Sebelumnya:</strong> {{ $validatorDokumen->user->name }}
                                            <br>
                                ...
                                <div class="alert alert-warning alert-permanent mb-0">
                                    <small><i class="bi bi-exclamation-triangle"></i> Belum ada validator. Silakan pilih validator baru.</small>
                                </div>
                                @endif
                            </div>
                        </div>
            ...
            <div class="card">
                <div class="card-header bg-white d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">Daftar Penugasan AK Banding</h5>
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover align-middle mb-0">
                            ...
                            <tbody>
                                ...
                                $suratTugas = $assignment->role_selected->name === 'asesor_banding'
                                ? $suratTugasAsesor
                                : $suratTugasValidator;

                                $jenisDokumen = $assignment->role_selected->name === 'asesor_banding'
                                    ...
                                    <td>
                                        ...
                                        <div class="d-flex align-items-center gap-2">
                                            ...
                                            <div class="btn-group btn-group-sm">
                                                <a href="{{ route('de.banding.penugasan-ak-banding.download-surat-tugas', [$pengajuan->id, $jenisDokumen]) }}" class="btn btn-sm btn-success" target="_blank" title="Download">
                                                    <i class="bi bi-eye"></i>
                                                </a>
            ...
            <div class="alert alert-warning alert-permanent">
                <h5><i class="bi bi-exclamation-triangle"></i> Belum Siap untuk AK Banding</h5>
                <p class="mb-0">Silakan tetapkan status "Siap untuk AK Banding" terlebih dahulu dengan mengklik tombol di kanan atas.</p>
            </div>
            @endif
            ...
            <div class="card my-4">
                <div class="card-header bg-primary text-white">
                    <h5 class="mb-0">Informasi Penugasan Asesor AK Banding</h5>
                </div>
                <div class="card-body">
                    <table class="table table-borderless">
                        <tr>
                            <th>Tanggal Penugasan Asesor AK Banding</th>
                            <td>
                                : {{ $pengajuan->tanggal_penugasan_asesor_ak_banding
                                    ? $pengajuan->tanggal_penugasan_asesor_ak_banding->locale('id')->translatedFormat('d M Y H:i')
                                    : '-' }}
                            </td>
                        </tr>
                        <tr>
                            <th>Tanggal Mulai AK Banding</th>
                            <td>
                                : {{ $pengajuan->asesmen?->asesmenKecukupanBanding?->tanggal_mulai
                                    ? \Carbon\Carbon::parse($pengajuan->asesmen->asesmenKecukupanBanding->tanggal_mulai)->locale('id')->translatedFormat('d M Y')
                                    : '-' }}
                            </td>
                        </tr>
                        <tr>
                            <th>Tanggal AK Banding Selesai</th>
                            <td>
                                : {{ $pengajuan->asesmen?->asesmenKecukupanBanding?->tanggal_selesai
                                    ? \Carbon\Carbon::parse($pengajuan->asesmen->asesmenKecukupanBanding->tanggal_selesai)->locale('id')->translatedFormat('d M Y')
                                    : '-' }}
                            </td>
                        </tr>
                        <tr>
                            <th>Status Penugasan Asesor AK Banding</th>
                            <td>: {!! $pengajuan->getCustomBadgeLastStatus('penugasan_asesor_ak_banding', 'de', 'label_long_for') !!}</td>
                        </tr>
                    </table>
                </div>
            </div>
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <table class="table table-borderless mb-3">
                        <tr>
                            <th style="width:60%"><i class="bi bi-person"></i> Asesor Banding</th>
        ...
        <div class="modal-content">
            ...
            <form id="formMarkReadyAK" onsubmit="submitMarkReadyAK(event)">
                <div class="modal-body">
                    <div class="alert alert-info alert-permanent mb-3">
                        <i class="bi bi-info-circle"></i>
                        <strong>Informasi:</strong> Tentukan periode pelaksanaan Asesmen Kecukupan (AK) Banding
                    </div>

                    <div class="mb-3">
                        <label class="form-label fw-bold">
                            Tanggal Mulai AK Banding <span class="text-danger">*</span>
                        </label>
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-bold">
                            Estimasi Tanggal Selesai AK Banding <span class="text-danger">*</span>
                        </label>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="alert alert-info alert-permanent mb-0">
                    ...
                    Jika Anda menggunakan validator yang sama, penugasan AK Banding akan langsung diterima (accepted) tanpa penawaran,
                    dan surat tugas akan otomatis sama dengan surat tugas validator di proses sebelumnya.
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    async function submitMarkReadyAK(event) {
        ...
        Swal.fire({
            title: 'Memproses...'
            , html: 'Sedang menetapkan status AK banding siap'
        ...
        try {
            const response = await fetch(`/de/banding/penugasan-ak-banding/${currentPengajuanId}/mark-ready`, {
                method: 'POST'
                , headers: {
    ...
    async function assignUser(event, pengajuanId) {
        ...
        try {
            const response = await fetch(`/de/banding/penugasan-ak-banding/${pengajuanId}/assign-user`, {
                method: 'POST'
                , headers: {
                    'X-CSRF-TOKEN': csrfToken
                    , 'Accept': 'application/json'
    ...
    async function removeUser(pengajuanId, userId, userName) {
        const result = await Swal.fire({
            ...
            , html: `Hapus <strong>${userName}</strong> dari penugasan AK Banding?`
            , icon: 'warning'
        ...
        try {
            const response = await fetch(`/de/banding/penugasan-ak-banding/${pengajuanId}/remove-user/${userId}`, {
                method: 'DELETE'
                , headers: {
                    'Content-Type': 'application/json'
                    , 'X-CSRF-TOKEN': csrfToken
    ...
    function showUploadSuratTugasModal(jenisDokumen) {
        ...
        form.action = `/de/banding/penugasan-ak-banding/${pengajuanId}/upload-surat-tugas/${jenisDokumen}`;
