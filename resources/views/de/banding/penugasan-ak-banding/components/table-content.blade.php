...
<div class="card">
    <div class="card-header bg-light d-flex justify-content-between align-items-center">
        <h6 class="mb-0">
            <i class="bi bi-table"></i> Daftar Penugasan Asesor AK Banding
        </h6>
    ...
    <div class="card-body p-0">
            ...
            <table class="table table-hover align-middle mb-0">
                <thead class="table-light">
                    <tr>
                        ...
                        <th width="15%">Status Penugasan Asesor AK Banding</th>
                ...
                <tbody>
                    ...
                    $asesorCount = $asesmenKecukupanBanding?->asesors->count() ?? 0;
                    $validatorCount = $asesmenKecukupanBanding?->validators->count() ?? 0;
                    ...
                    $preAkStatuses = [
                    \App\Models\PengajuanAkreditasi::STATUS_PEMBAYARAN_BANDING_DIVERIFIKASI,
                    ];
                    ...
                    // ===== Badge Status AK =====
                    if ($akStatus === App\Models\PengajuanAkreditasi::STATUS_AK_BANDING_DILAPORKAN) {
                    $statusAK = ['class' => 'success', 'icon' => 'check-circle-fill', 'text' => 'Selesai'];
                    } elseif ($akStatus === App\Models\PengajuanAkreditasi::STATUS_AK_BANDING_SELESAI) {
                    $statusAK = ['class' => 'success', 'icon' => 'check-circle', 'text' => 'AK Banding Selesai'];
                    } elseif ($akStatus === App\Models\PengajuanAkreditasi::STATUS_AK_BANDING_ON_VALIDATION) {
                    $statusAK = ['class' => 'warning', 'icon' => 'shield-check', 'text' => 'Sedang Validasi'];
                    } elseif ($akStatus === App\Models\PengajuanAkreditasi::STATUS_AK_BANDING_IN_PROGRESS) {
                    $statusAK = ['class' => 'info', 'icon' => 'hourglass-split', 'text' => 'Sedang Proses'];
                    } elseif ($akStatus === App\Models\PengajuanAkreditasi::STATUS_ASESOR_AK_BANDING_ASSIGNED) {
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <div class="mb-1">
                                <small><i class="bi bi-person"></i> Asesor Banding: <strong>{{ $asesorCount }}</strong></small>
                            </div>
                            <div>
                        ...
                        <td>
                            {!! $pengajuan->getCustomBadgeLastStatus('penugasan_asesor_ak_banding','de','label_short_for') !!}
                        </td>
                        <td>
                            @if($pengajuan->tanggal_penugasan_asesor_ak_banding)
                            <small>{{ $pengajuan->tanggal_penugasan_asesor_ak_banding->locale('id')->translatedFormat('d M Y') }}</small>
                            <br>
                            <small class="text-muted">
                                {{ $pengajuan->tanggal_penugasan_asesor_ak_banding->diffForHumans() }}
                            </small>
                            @else
                        ...
                        <td class="text-center">
                            <a href="{{ route('de.banding.penugasan-ak-banding.show', $pengajuan->id) }}" class="btn btn-sm btn-primary" title="Lihat Detail & Tugaskan">
                                <i class="bi bi-eye"></i>
                            </a>
                        </td>
                    </tr>
        ...
        <div class="text-center py-5">
            ...
            <p class="text-muted mt-3 mb-0">Tidak ada permohonan akreditasi yang sudah berada pada tahap AK banding</p>
        </div>
        @endif
    </div>
