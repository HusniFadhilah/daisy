...
<div class="container-fluid">
    ...
    <div class="row mb-4">
        ...
        {{--
            ✅ PERUBAHAN: Peringkat Akreditasi
            - Hapus $badgeClass match() hardcode yang tidak terpakai
            - getPeringkatColor() tanpa parameter (warna diambil dari relasi statusFinal/statusAl/statusAk)
        --}}
        <div class="col-md-3">
            <div class="card border-0 shadow-sm h-100">
                <div class="card-body text-center">
                    ...
                    <span class="badge p-2 px-3 my-4 fs-5" style="background-color: {{ $hasil->getPeringkatColor() }}; color:#222">
    ...
    <div class="card mb-4 border-0 shadow-sm">
        ...
        <div class="card-body">
            <div class="row g-2">
                @foreach($statusAkreditasiList as $sa)
                <div class="col-md-6">
                    <div class="d-flex align-items-center p-3 rounded h-100" style="background-color: {{ $sa->warna ?? '#e2e3e5' }}22; border-left: 4px solid {{ $sa->warna ?? '#e2e3e5' }};">
                        <div class="me-3 text-nowrap">
                            <strong class="text-dark">
                                {{ $sa->skor_min }} - {{ $sa->skor_max }}
                            </strong>
                        </div>
                        <div class="flex-grow-1">
                            <strong class="text-dark">{{ $sa->status }}</strong>
                            @if($sa->siklus_tahun)
                            <div><small class="text-muted">({{ $sa->siklus_tahun }} Tahun)</small></div>
                            @endif
                            @if($sa->keterangan)
                            <div><small class="text-muted">{{ $sa->keterangan }}</small></div>
                            @endif
                        </div>
                    </div>
                </div>
    ...
    {{--
        ✅ PERUBAHAN: Validasi Syarat Unggul
        Sebelumnya:
          - Kondisi hardcode: $hasil->skor_final >= 361
          - Loop kriteria dari konstanta statis: HasilAkreditasi::KRITERIA_REQUIRED
        Sesudah:
          - Kondisi dinamis: $validationSummary['skor_memenuhi'] (threshold dari DB via repository)
          - Loop kriteria dari: array_keys($validationSummary['kriteria_status']) (dinamis dari DB)
          - Tambah syarat P.1: rasio DTPS:Mahasiswa dan jabatan Lektor ke atas
    --}}
    @if($validationSummary['skor_memenuhi'])
    @php $sp = $validationSummary['syarat_p1']; @endphp
    <div class="card mb-4 border-{{ $validationSummary['dapat_unggul'] ? 'success' : 'warning' }}">
        ...
        <div class="card-body">
            ...
            <div class="row mb-3 g-3">
                ...
                {{-- Syarat 1: Skor --}}
                <div class="col-md-6">
                    <div class="d-flex align-items-center p-3 bg-success bg-opacity-10 rounded h-100">
                ...
                <div class="col-md-6">
                    <div class="d-flex align-items-center p-3 bg-{{ $validationSummary['pelampauan_memenuhi'] ? 'success' : 'danger' }} bg-opacity-10 rounded h-100">
                ...
                <div class="col-md-6">
                    <div class="d-flex align-items-center p-3 bg-{{ $sp['rasio']['memenuhi'] ? 'success' : 'danger' }} bg-opacity-10 rounded h-100">
                        <i class="bi bi-{{ $sp['rasio']['memenuhi'] ? 'check-circle-fill text-success' : 'x-circle-fill text-danger' }} fs-3 me-3"></i>
                        <div>
                            <strong>Rasio DTPS : Mahasiswa</strong>
                            <div class="text-muted">{{ $sp['rasio']['keterangan'] }}</div>
                            @if(!is_null($sp['rasio']['rasio'] ?? null))
                            <div class="text-muted mt-1" style="font-size:.8rem">
                                DTPS: {{ $sp['rasio']['jumlah_dtps'] }} &middot;
                                Mahasiswa: {{ $sp['rasio']['jumlah_mahasiswa'] ?? '—' }} &middot;
                                Rumpun: <span class="text-capitalize">{{ $sp['rasio']['rumpun'] }}</span>
                            </div>
                            @else
                ...
                <div class="col-md-6">
                    <div class="d-flex align-items-center p-3 bg-{{ $sp['jabatan']['memenuhi'] ? 'success' : 'danger' }} bg-opacity-10 rounded h-100">
                        <i class="bi bi-{{ $sp['jabatan']['memenuhi'] ? 'check-circle-fill text-success' : 'x-circle-fill text-danger' }} fs-3 me-3"></i>
                        <div>
                            <strong>Jabatan Lektor ke Atas ≥ {{ $sp['jabatan']['persen_minimum'] ?? 50 }}%</strong>
                            <div class="text-muted">{{ $sp['jabatan']['keterangan'] }}</div>
                            @if(($sp['jabatan']['total_dtps'] ?? 0) > 0)
                            <div class="text-muted mt-1" style="font-size:.8rem">
                                {{ $sp['jabatan']['lektor_ke_atas'] }} dari {{ $sp['jabatan']['total_dtps'] }} DTPS
                                ({{ $sp['jabatan']['persen_lektor'] }}%)
                            </div>
                            @else
                            <div class="text-warning mt-1" style="font-size:.8rem">
            ...
            <div class="alert alert-warning alert-permanent mt-3 mb-0">
                ...
                <ul class="mb-0 mt-2">
                    ...
                    <li>{{ $sp['rasio']['keterangan'] }}</li>
                    @endif
                    @if(!$sp['jabatan']['memenuhi'])
                    <li>{{ $sp['jabatan']['keterangan'] }}</li>
                    @endif
                </ul>
            </div>
            @endif
