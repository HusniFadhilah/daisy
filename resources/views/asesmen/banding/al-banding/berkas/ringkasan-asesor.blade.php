        ...
        function bindUploader(cfg) {
            ...
            function handle() {
                ...
                if (file.type !== 'application/pdf') {
                    Swal.fire('Perhatian', 'File harus PDF', 'warning');
                    reset();
                    return;
                }
                if (file.size > 20 * 1024 * 1024) {
                    Swal.fire('Perhatian', 'Ukuran file maksimal 20MB', 'warning');
                    reset();
                    return;
                }
            ...
            form.addEventListener('submit', function(e) {
                ...
                fetch(cfg.uploadUrl, {
                    ...
                    .then(function(res) {
                        if (res.success) {
                            Swal.fire('Berhasil', res.message || 'Upload berhasil!', 'success');
                            window.location.reload();
                        } else {
                            Swal.fire('Perhatian', res.message || 'Gagal upload', 'error');
                            btnSubmit.disabled = false;
                    ...
                    .catch(function(error) {
                        ...
                        Swal.fire('Perhatian', 'Terjadi kesalahan saat upload', 'error');
                        btnSubmit.disabled = false;
                        btnSubmit.innerHTML = '<i class="bi bi-upload"></i> Upload';
                    });
            });
        ...
        delBtns.forEach(function(btn) {
            btn.addEventListener('click', async function() {
                ...
                fetch(deleteUrl, {
                    ...
                    .then(function(res) {
                        if (res.success) {
                            Swal.fire('Berhasil', res.message || 'Dokumen berhasil dihapus', 'success');
                            window.location.reload();
                        } else {
                            Swal.fire('Perhatian', res.message || 'Gagal hapus dokumen', 'error');
                            window.location.reload();
                        }
                    })
                    .catch(function(error) {
