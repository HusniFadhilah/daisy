                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <div class="btn-group btn-group-sm" role="group">
                                            <a href="{{ route('al_banding.berkas.documents.preview', ['id' => $asesmen->id, 'docId' => $doc->id]) }}" class="btn btn-outline-primary" target="_blank" title="Lihat File">
                                                <i class="bi bi-eye"></i>
                                            </a>

                                            {{-- ✅ Tombol hapus hanya muncul jika user adalah uploader --}}
                                            @if($isUploader && $doc->uploaded_by == Auth::id())
                                            <button type="button" class="btn btn-outline-danger btn-delete-doc" data-doc-id="{{ $doc->id }}" data-doc-name="{{ $doc->title }}" data-url="{{ route('al_banding.berkas.documents.delete', ['id' => $asesmen->id, 'docId' => $doc->id]) }}" title="Hapus">
                    ...
                    <ol class="mb-4 small ps-3">
                        <li class="mb-2">
                            ...
                            <div class="my-2 text-center">
                                <a href="{{ route('al_banding.berkas.export', ['idAsesmen' => $asesmen->id, 'mode' => 'personal']) }}" class="btn btn-sm btn-outline-primary">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (uploadForm) {
            uploadForm.addEventListener('submit', function(e) {
                ...
                fetch("{{ route('al_banding.berkas.documents.upload', ['id' => $asesmen->id]) }}", {
                        method: 'POST'
                        , body: formData
                        , headers: {
        ...
        if (btnFinalize) {
            btnFinalize.addEventListener('click', async function() {
                ...
                fetch("{{ route('al_banding.berkas.documents.finalize', ['id' => $asesmen->id]) }}", {
                        method: 'POST'
                        , headers: {
                            'X-CSRF-TOKEN': "{{ csrf_token() }}"
                            , 'Accept': 'application/json'
