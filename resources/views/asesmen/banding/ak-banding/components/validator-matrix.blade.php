            ...
            <div class="btn-group btn-group-sm btn-sm ms-md-auto">
                <div class="dropdown">
                    <button class="btn btn-success dropdown-toggle" type="button" id="btnDownloadPenilaian" data-bs-toggle="dropdown" aria-expanded="false" title="Download Penilaian">
                        <i class="bi bi-download"></i> Download Validasi AK Banding
                    </button>
                    <ul class="dropdown-menu dropdown-wrap" aria-labelledby="btnDownloadPenilaian">
                        <li>
                            <a class="dropdown-item" href="{{ route('ak_banding.validasi.export.comparison', ['asesmen'=>$asesmen->id, 'mode'=>'split']) }}">
                                Download tampilan Terpisah (pisah kolom Pemenuhan & Pelampauan)
                            </a>
                        </li>
                        <li>
                            <a class="dropdown-item" href="{{ route('ak_banding.validasi.export.comparison', ['asesmen'=>$asesmen->id, 'mode'=>'merged']) }}">
    ...
    <div class="card-body p-0">
        ...
        <div class="validator-matrix-wrapper" style="overflow-x: auto; overflow-y: auto; max-height: 800px;">
            <table class="validator-matrix-table {{ $viewMode === 'merged' ? 'matrix-merged' : 'matrix-split' }}" id="validatorMatrix">
                ...
                $penilaianElemenAkBanding = $elemen->penilaianElemenAkBanding->where('id_asesor', $asesor->id_user)->first();
                if ($penilaianElemenAkBanding && $penilaianElemenAkBanding->skor !== null) {
                $skors[] = $penilaianElemenAkBanding->skor;
                }
                }
                ...
                <tbody class="validator-group" data-has-diff="{{ $groupHasDiff ? 'true' : 'false' }}">
                    ...
                    foreach ($asesors as $asesor) {
                    $penilaianElemenAkBanding = $elemen->penilaianElemenAkBanding->where('id_asesor', $asesor->id_user)->first();
                    $penilaians[$asesor->id_user] = $penilaianElemenAkBanding;
                    if ($penilaianElemenAkBanding && $penilaianElemenAkBanding->skor !== null) {
                    $skors[] = $penilaianElemenAkBanding->skor;
                    }
                    }
                    ...
                    $validasi = $elemen->penilaianElemenAkBanding->whereNotIn('status_validasi', ['pending','not_validated'])->first();
                    @endphp
