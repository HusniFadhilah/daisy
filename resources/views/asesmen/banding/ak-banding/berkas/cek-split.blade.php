{{-- resources\views\asesmen\banding\ak-banding\berkas\cek-split.blade.php --}}
@extends('layouts.template.app')

@section('title', 'Cek Split Penilaian AK Banding - ' . $asesmen->name)
    ...
    <div class="card mb-4 shadow-sm">
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-start">
                <div>
                    <h4 class="mb-1">
                        <i class="bi bi-people"></i> Cek Split Penilaian Antar Asesor Banding
                    </h4>
                    <p class="text-muted mb-0">{{ $asesmen->getName(false) }}</p>
                </div>
                <div class="d-flex gap-2">
                    <a href="{{ route('ak_banding.berkas.export', ['idAsesmen' => $asesmen->id, 'mode' => 'split', 'color' => false]) }}" class="btn btn-outline-success">
...
<script>
    ...
    async function loadComparisonData() {
        ...
        try {
            const response = await fetch(`/ak-banding/berkas/${idAsesmen}/comparison-data`, {
                headers: {
    ...
    function populateComparisonModal(data) {
        ...
        asesors.forEach((asesor, index) => {
            ...
            <th class="comp-header comp-sticky-header text-center"
                ...
                <div class="d-flex flex-column align-items-center">
                    <div class="fw-bold text-white">Penilaian Asesmen Kecukupan Banding Asesor ${asesor.urutan_asesor}</div>
        ...
        kriterias.forEach(kriteria => {
            ...
            kriteria.elemen_standar.forEach(elemen => {
                ...
                const penilaianData = asesors.map(asesor => {
                    const penilaian = elemen.penilaian_elemen_ak_banding.find(p => p.id_asesor === asesor.id_user);
                    return {
                ...
                // Click handler for split rows → navigate to show berkas at elemen anchor
                const onclickRow = isSplit ?
                    `onclick="window.open('/ak-banding/berkas/${idAsesmen}#elemen-${elemen.id}', '_blank')"` :
                    '';

                bodyHtml += `<tr class="comparison-row" data-elemen-id="${elemen.id}" data-is-split="${isSplit}" ${onclickRow}>`;
                ...
                asesors.forEach(asesor => {
                    ...
                    const onclickMerged = (skor !== null) ?
                        `onclick="event.stopPropagation(); showKomentarPopover(this, '${asesor.user.name}', ${skor}, '${safeKomentar}')"` :
                        '';

                    bodyHtml += `
                    <td class="comp-cell comp-score-cell comp-score-merged" style="background: ${bgMerged};" ${onclickMerged}>
                        ${skor !== null
                            ? `<small class="text-dark">${komentar || 'Tidak ada komentar'}</small>`
                ...
                const splitBadge = isSplit ?
                    '<span class="badge bg-danger" style="width:50px;font-size:13px">Ya</span>' :
                    '<span class="badge bg-success" style="width:50px;font-size:13px">Tidak</span>';
                ...
                const ketSplitHtml = isSplit && splitInfo.keterangan ?
                    `<span>${splitInfo.keterangan}</span>
                       <div class="mt-1"><small class="text-primary"><i class="bi bi-box-arrow-up-right"></i> Klik baris untuk detail</small></div>` :
        ...
        // Update statistics with recalculated split count
    ...
    function setupHighlightSplitButton() {
        ...
        if (!btnHighlight || btnHighlight._bound) return;
