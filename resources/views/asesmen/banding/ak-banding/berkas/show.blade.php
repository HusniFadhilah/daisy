...
@section('title', 'Penilaian AK Banding - ' . $asesmen->name)
...
<div class="container-fluid py-3">
    ...
    <div class="card mb-4">
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <div>
                    <h4 class="mb-1">Penilaian Asesmen Kecukupan Banding</h4>
                    <p class="text-muted mb-0">{{ $asesmen->getName(false) }}</p>
                </div>
                <a href="{{ route('ak_banding.berkas') }}" class="btn btn-outline-secondary">
                        ...
                        <div class="d-flex align-items-start">
                            ...
                            <div class="flex-grow-1">
                                ...
                                <p class="mb-2">
                                    ...
                                    Anda dapat melakukan cek split penilaian antar asesor di tombol berikut <a class="btn btn-info btn-sm" href="{{ route('ak_banding.berkas.cek-split', $asesmen->id) }}" target="_blank">
                    ...
                    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start gap-3">
                        ...
                        <div class="btn-group flex-wrap w-md-100 w-md-auto">
                            ...
                            <ul class="dropdown-menu">
                                ...
                                <!-- Download Templat -->
                                <li>
                                    <a class="dropdown-item" href="{{ route('ak_banding.berkas.export', ['idAsesmen' => $asesmen->id, 'mode' => 'template']) }}" id="btnDownloadTemplate">
                                ...
                                <li>
                                    <a class="dropdown-item btnDownloadData" data-mode="full" href="{{ route('ak_banding.berkas.export', ['idAsesmen' => $asesmen->id, 'mode' => 'full']) }}">
                                ...
                                <li>
                                    <a class="dropdown-item btnDownloadData" data-mode="personal" href="{{ route('ak_banding.berkas.export', ['idAsesmen' => $asesmen->id, 'mode' => 'personal']) }}">
    ...
    @include('asesmen.banding.ak-banding.components.heatmap-matrix')
            ...
            <div id="collapse-kriteria-{{ $kriteria->id }}" class="accordion-collapse collapse kriteria-collapse" aria-labelledby="heading-kriteria-{{ $kriteria->id }}" data-bs-parent="#accordionKriteria">
                <div class="card-body">
                    ...
                    <div class="accordion accordion-elemen" id="accordionElemen-{{ $kriteria->id }}">
                        ...
                        $penilaianElemenAkBanding = $elemen->penilaianElemenAkBanding->first(); // Assuming relation exists
                        $hasPenilaian = $penilaianElemenAkBanding && $penilaianElemenAkBanding->skor !== null;
                        $needsRevisionElemen = $penilaianElemenAkBanding && $penilaianElemenAkBanding->status_validasi === 'revision_required';
                        ...
                        $validatedBy = $penilaianElemenAkBanding->validator->name ?? 'N/A';
                        $isIndikatorPenilaianExists = $elemen->indikatorPenilaian && count($elemen->indikatorPenilaian) > 0;
                        @endphp

                        <div class="card mb-3 elemen-card @if($hasPenilaian) has-penilaian @endif" data-elemen-id="{{ $elemen->id }}" id="elemen-{{ $elemen->id }}">
                                ...
                                <div class="penilaian-form-wrapper mb-4">
                                    <div class="card border-{{ $needsRevisionElemen ? 'warning' : ($hasPenilaian ? 'success' : 'warning') }}">
                                        ...
                                        <div class="card-body">
                                            ...
                                            @if($needsRevisionElemen)
                                            @php
                                            $preferensiSkor = $penilaianElemenAkBanding->preferensi_skor;
                                            @endphp
                                            <div class="alert alert-warning alert-permanent alert-dismissible mb-3">
                                                <div class="row">
                                                    <div class="col-md-8">
                                                        ...
                                                        <p class="mb-2"><strong>"{{ $penilaianElemenAkBanding->catatan_validator }}"</strong></p>
                                                    </div>
                                                ...
                                                <div class="row mt-3">
                                                    ...
                                                    <div class="col-md-6 text-end">
                                                        <small class="text-muted">
                                                            <i class="bi bi-clock"></i> <strong>Tanggal:</strong> {{ \App\Libraries\Date::tglWaktu($penilaianElemenAkBanding->validated_at) }}
                                                        </small>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            @php
                                            $notAllowedEdit = $hasPenilaian && in_array($penilaianElemenAkBanding->status_validasi, ['validated', 'validated_diff', 'approved'])
                                            @endphp
                                            <form class="form-penilaian" data-elemen-id="{{ $elemen->id }}">
                                                <div class="row mb-3">
                                                    <div class="col-md-12 mb-3">
                                                        ...
                                                        <select class="form-select skor-select" name="skor" required>
                                                            ...
                                                            @foreach ($jenjangs as $jenjang)
                                                            <option value="{{ $jenjang->skor }}" @if($hasPenilaian && $penilaianElemenAkBanding->skor == $jenjang->skor) selected @endif style="background:{{ $jenjang->color }}; color:{{ \App\Models\JenjangPenilaian::textColorByBg($jenjang->color) }}">
                                                                {{ $jenjang->skor }} - {{ $jenjang->name }}
                                                            </option>
                                                            @endforeach
                                                        </select>
                                                    ...
                                                    <div class="col-md-12 komentar-section">
                                                        ...
                                                        <textarea class="form-control komentar-textarea" name="komentar" rows="15" placeholder="Berikan justifikasi dan analisis penilaian berdasarkan seluruh indikator di bawah ini..." maxlength="2000" required {{ $notAllowedEdit ? 'disabled':'' }}>{{ $hasPenilaian ? $penilaianElemenAkBanding->komentar : '' }}</textarea>
                                                        <small class="text-muted">
                                                            <i class="bi bi-info-circle me-1" title="Batas maksimal komentar adalah 2000 karakter"></i>
                                                            <span class="char-count">{{ $hasPenilaian ? strlen($penilaianElemenAkBanding->komentar) : 0 }}</span> / 2000 karakter
                                                        </small>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="d-flex flex-column flex-md-row align-items-start align-items-md-center gap-2">
                                                    <div class="save-status text-muted small d-flex align-items-center flex-wrap">
                                                        ...
                                                        <span class="status-text">
                                                            @if($hasPenilaian)
                                                            Tersimpan pada {{ \App\Libraries\Date::tglWaktu($penilaianElemenAkBanding->updated_at) }}
                                                            @else
                                                            Belum ada penilaian
                                                            @endif
                                                        </span>
                                                ...
                                                <div class="alert alert-info alert-permanent">
                                                    ...
                                                    <small>
                                                        Telah divalidasi oleh <strong>{{ $validatedBy }}</strong>
                                                        pada {{ \App\Libraries\Date::tglWaktu($penilaianElemenAkBanding->validated_at) }}
                                                    </small>
                                                </div>
                                                @endif
                                            </form>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        async function submitPenilaian() {
            ...
            try {
                const response = await fetch(`/ak-banding/berkas/${idAsesmen}/submit`, {
                    method: 'POST'
                    , headers: {
                ...
                try {
                    const splitRes = await fetch(`/ak-banding/berkas/${idAsesmen}/check-split-result`, {
                        headers: {
                            'Accept': 'application/json'
                            , 'X-CSRF-TOKEN': '{{ csrf_token() }}'
                        , }
                ...
                <div class="text-center">
                    ...
                    <div class="alert alert-info text-start mt-3">
                        ...
                        <br><br>
                        <a href="/ak-banding/berkas/${idAsesmen}/cek-split" target="_blank"
            ...
            `
                ...
                const showBerkasBase = `/ak-banding/berkas/${idAsesmen}`;
        ...
        async function unSubmitPenilaian() {
            ...
            try {
                const response = await fetch(`/ak-banding/berkas/${idAsesmen}/unsubmit`, {
                    method: 'POST'
                    , headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                        , 'Accept': 'application/json'
        ...
        async function exportExcel() {
            ...
            try {
                window.location.href = `/ak-banding/berkas/${idAsesmen}/export`;
        ...
        async function importExcel(e) {
            ...
            try {
                // Upload file
                const response = await fetch(`/ak-banding/berkas/${idAsesmen}/import`, {
                    method: 'POST'
                    , body: formData
                    , headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
        ...
        async function autoSavePenilaian(form, idElemen) {
            ...
            isSaving = true;
            try {
                const response = await fetch(`/ak-banding/berkas/${idAsesmen}/nilai`, {
                    method: 'POST'
                    , headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                        , 'Content-Type': 'application/json'
        ...
        async function savePenilaian(form, idElemen) {
            ...
            try {
                ...
                if (!response.ok && data.error_type === 'already_validated') {
                    await Swal.fire({
                        icon: 'warning'
                        , title: 'Penilaian Telah Divalidasi'
                        , html: `
                    <div class="text-start">
                        <p><strong>Penilaian ini telah divalidasi dan disetujui oleh validator.</strong></p>
                        ...
                        <p class="text-muted mb-0">
                            Penilaian yang telah divalidasi tidak dapat diubah untuk menjaga integritas hasil asesmen.
                        </p>
                    </div>
                `
                        , confirmButtonText: 'Mengerti'
        ...
        function downloadTemplate() {
            ...
            window.location.href = `/ak-banding/berkas/${idAsesmen}/template`;
        ...
        function downloadDataExcel(e) {
            ...
            window.location.href = `/ak-banding/berkas/${idAsesmen}/export?mode=${mode}`;
        ...
        async function importHistoryExcel() {
            ...
            // Load history
            try {
                const response = await fetch(`/ak-banding/berkas/${idAsesmen}/import-history`, {
                    headers: {
                        'Accept': 'application/json'
                        , 'X-CSRF-TOKEN': '{{ csrf_token() }}'
                    }
        ...
        `
            ...
            try {
                const response = await fetch(`/ak-banding/berkas/${idAsesmen}/reset-all`, {
                    method: 'DELETE'
                    , headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                        , 'Accept': 'application/json'
    ...
    function showValidatedWarning(data, form, idElemen) {
        ...
        <strong>Penilaian Terkunci</strong><br>
        <small>
            Telah divalidasi oleh <strong>${data.validator_name}</strong> pada ${formatDateTime(data.validated_at)}
        </small>
    `;
...
@endpush
@include('asesmen.banding.ak-banding.components.modal-comparison-asesor')
@endsection
