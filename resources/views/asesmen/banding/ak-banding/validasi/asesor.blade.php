{{-- resources/views/asesmen/banding/ak-banding/validasi/asesor.blade.php --}}

@extends('layouts.template.app')

@section('title', 'Validasi Penilaian Asesor Banding - ' . $asesmen->name)
...
<div class="container-fluid py-3">
    ...
    <div class="card mb-4 header-card shadow-sm">
        <div class="card-body">
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-3 gap-2">
                ...
                <div class="flex-grow-1">
                    <h3 class="mb-1 text-wrap text-break">Validasi Penilaian Asesor Banding</h3>
                ...
                <!-- Tombol -->
                <a href="{{ route('ak_banding.validasi.index') }}" class="btn btn-outline-secondary mt-2 mt-md-0">
                            ...
                            <h2 class="accordion-header" id="headingAsesor{{ $index }}">
                                <button class="accordion-button collapsed bg-{{ $color['bg'] }} text-white d-flex align-items-center" type="button" data-bs-toggle="collapse" data-bs-target="#collapseAsesor{{ $index }}" aria-expanded="false" aria-controls="collapseAsesor{{ $index }}">
                                    ...
                                    <div class="d-flex align-items-center gap-2">
                                        ...
                                        <span>Asesor Banding {{ $asesor->urutan_asesor }}</span>
                                    </div>
                                ...
                                <div class="accordion-body">
                                    ...
                                    <div class="d-flex align-items-center mb-3">
                                        ...
                                        <div>
                                            ...
                                            <span class="badge bg-light text-dark d-block text-wrap">
                                                {{ $asesor->user->email }}
                                            </span>
                                        </div>
                                    </div>
        ...
        <div class="card-footer bg-white py-3">
            ...
            $labelBtn = $jenisPelaporan === 'ak_banding' ? 'Laporan Penilaian Kecukupan LED Program Studi (LHK) Banding' : 'Pelaporan AL Banding';
            @endphp
            ...
            <div class="alert alert-success alert-permanent mb-3">
                ...
                <p class="mb-0">Validasi {{ $sudahDilaporkan ? 'dan Pelaporan AK Banding ' : '' }}telah diselesaikan dan lolos untuk tahap selanjutnya (Asesmen Lapangan/AL Banding). {!! !$sudahDilaporkan ? '<br>Mohon membuat Laporan Penilaian Kecukupan LED Program Studi (LHK) Banding pada menu Pelaporan AK Banding' : '' !!}</p>
            </div>
            @endif
    ...
    {{-- Validator Matrix --}}
    @include('asesmen.banding.ak-banding.components.validator-matrix', ['asesors' => $asesors,'kriterias' => $kriterias,'jenjangs' =>$jenjangs])
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    <button type="button" class="btn btn-outline-success w-100" id="btnValidateAllAgreed" {{ $isApproved ? 'disabled' : '' }}>
                        ...
                        <small>Otomatis approve nilai yang sama dari semua asesor banding</small>
                    </button>
                </div>
                {{-- @endif --}}
                <div class="col-md-6">
                    <button type="button" class="btn btn-outline-warning w-100" id="btnReviewDifferences">
                        ...
                        <small>Lihat hanya penilaian yang berbeda antar asesor banding</small>
                    </button>
                </div>
            </div>
        </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="alert alert-warning alert-permanent">
                    <i class="bi bi-exclamation-triangle"></i>
                    <strong>Perhatian:</strong> Setelah disetujui, asesor banding tidak dapat mengubah penilaian mereka.
                </div>
...
{{-- Include Modals --}}
@include('asesmen.banding.ak-banding.components.modal-validasi-detail')
@include('asesmen.banding.ak-banding.components.modal-detail-validasi')
...
<script>
    ...
    window.PELAPORAN_CFG.ak = {
        title: 'Rekap AK dan Validasi AK Banding'
        , label: 'Laporan Penilaian Kecukupan LED Program Studi (LHK) Banding'
        , upload: @json(route('pelaporan.banding.validasiAk.upload', ['assignment' => '__ID__']))
        , finalize: @json(route('pelaporan.banding.validasiAk.finalize', ['assignment' => '__ID__']))
        , fileLabel: 'Laporan Penilaian Kecukupan LED Program Studi (LHK) telah selesai'
        , finalizeLabel: 'Asesmen Kecukupan Banding telah Dilaporkan'
        ...
        •	Penyampaian Informasi Kepada DE untuk dilakukan tahap Asesmen Lapangan Banding`
    };
...
<script>
                        ...
                        <div class="d-flex align-items-center justify-content-between">
                            <div class="d-flex align-items-center">
                                ...
                                <div class="text-white">
                                    ...
                                    <small>Asesor Banding ${index + 1}</small>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-md-6 mb-2">
                    <div class="form-check">
                        ...
                        <label class="form-check-label" for="revisi_asesor_${asesor.id}">
                            <span class="badge" style="background: ${color};">Asesor Banding ${index + 1}</span>
                            ${asesor.name}
                        </label>
                    </div>
                </div>
    ...
    async function openValidationModal(elemenId) {
        ...
        try {
            const response = await fetch(`/ak-banding/validasi/${idAsesmen}/elemen/${elemenId}`, {
                headers: {
    ...
    document.addEventListener('DOMContentLoaded', function() {
            ...
            if (btnValidateAllAgreed) {
                btnValidateAllAgreed.addEventListener('click', async function() {
                    const confirmed = await Swal.fire({
                        ...
                        , text: 'Sistem akan otomatis menyetujui semua penilaian yang nilainya sama dari semua asesor banding.'
            ...
            if (btnReviewDifferences) {
                btnReviewDifferences.addEventListener('click', function() {
                    ...
                    if (!reviewDiffActive) {
                        ...
                        this.innerHTML = '<i class="bi bi-exclamation-triangle"></i><div>Review Perbedaan</div><small>Lihat hanya penilaian yang berbeda antar asesor banding</small>';
        ...
        function initializeApproveAll() {
            ...
            if (confirmApproveAll) {
                confirmApproveAll.addEventListener('click', async function() {
                    ...
                    try {
                        const response = await fetch(`/ak-banding/validasi/${idAsesmen}/asesor/approve`, {
                            method: 'POST'
                            , headers: {
        ...
        async function submitValidasi() {
            ...
            if (statusValidasi === 'revision_required') {
                ...
                if (selectedChecks.length === 0) {
                    Swal.fire({
                        icon: 'warning'
                        , title: 'Pilih Asesor Banding'
                        , text: 'Silakan centang asesor banding mana yang harus merevisi penilaian!'
                    });
                    return;
                }
            ...
            try {
                const response = await fetch(`/ak-banding/validasi/${idAsesmen}/elemen/${elemenId}/validate`, {
                    method: 'POST'
                    , headers: {
                        'Content-Type': 'application/json'
                        , 'Accept': 'application/json'
        ...
        async function validateAllAgreed() {
            ...
            try {
                const response = await fetch(`/ak-banding/validasi/${idAsesmen}/validate-agreed`, {
                    method: 'POST'
                    , headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                        , 'Accept': 'application/json'
    ...
    async function showValidasiDetail(elemenId, validasiId) {
        ...
        try {
            const response = await fetch(`/ak-banding/validasi/${idAsesmen}/detail/${elemenId}`, {
                headers: {
                    'Accept': 'application/json'
                    , 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                }
    ...
    function populateValidasiDetail(data) {
        ...
        const statusBadge =
            ...
            '<span class="badge bg-light text-dark">✅ Disetujui</span>' :
            validasi.status_validasi === 'validated_diff' ?
            '<span class="badge bg-light text-dark">ℹ️ Disetujui (Berbeda)</span>' :
            '<span class="badge bg-light text-dark">⚠️ Perlu Revisi</span>';
        ...
        if (validasi.skor_final && envIsLocal) {
            ...
            if (skorBadge) {
                skorBadge.textContent = getSkorLabel(validasi.skor_final) || '-';
                skorBadge.className = `badge text-dark`;
                skorBadge.style.backgroundColor = getSkorColorJS(validasi.skor_final);
            }
        } else {
            document.querySelectorAll('.detailValidasiSkor').forEach(e => {
                        ...
                        <div class="card-body">
                            <div class="d-flex align-items-center mb-2">
                                ...
                                <div>
                                    ...
                                    <div><small class="text-muted">Asesor Banding ${index + 1}</small></div>
                                </div>
                            </div>
