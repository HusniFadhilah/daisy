{{-- resources\views\keuangan\pembayaran\show.blade.php --}}
...
<div class="container-fluid py-3">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-md-between align-items-md-center mb-4 gap-2">
        ...
        <span class="badge bg-secondary align-self-md-center">
            Status Pembayaran:
            {{ strtoupper($pembayaran->status_pembayaran_label ?? '-') }}
        </span>
    </div>

    {{--
        $pembayaran sudah di-pass langsung dari controller (PengajuanPembayaran).
        $path, $ext, $url juga sudah di-pass dari controller.
        Tidak perlu lagi @php reassign di sini.
    --}}
    ...
    <div class="row">
        ...
        <div class="col-lg-7">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <div class="alert alert-info alert-permanent">
                        ...
                        @if($pembayaran->jenis_pembayaran === 'banding')
                        <strong>Jenis:</strong> <span class="badge bg-info">Pembayaran Banding</span><br>
                        @endif
                        <strong>Jumlah:</strong>
                        Rp {{ number_format($pembayaran->jumlah_pembayaran ?? 0, 0, ',', '.') }}<br>
                        <strong>Jatuh Tempo:</strong>
                        {{ \App\Libraries\Date::tglIndo($pembayaran->tanggal_jatuh_tempo) }}<br>
            ...
            {{-- ===== Aksi Verifikasi ===== --}}
            {{--
                ✅ Route verify sekarang pakai $pembayaran->id (PengajuanPembayaran.id),
                bukan $pengajuan->id
            --}}
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('keuangan.pembayaran.verify', $pembayaran->id) }}">
                        @csrf
            ...
            @elseif($pembayaran->status_pembayaran === 'menunggu_pembayaran')
            ...
            <div class="alert alert-danger alert-permanent">
                ...
                Pembayaran ini ditolak. Bagian keuangan meminta prodi untuk mengupload ulang formulir dan bukti pembayaran
            </div>
        ...
        <div class="col-lg-5">
            ...
            <div class="card mb-4">
                <div class="card-header bg-light">
                    <h5 class="mb-0">
                        <i class="bi bi-file-earmark-image"></i> Formulir & Bukti Pembayaran
                    </h5>
                </div>

                <div class="card-body">
                    @if(!$path)
                    <p class="text-muted mb-0">Formulir & Bukti Pembayaran belum diupload.</p>
                    @else
                    {{-- ✅ Route download sekarang pakai $pembayaran->id --}}
                    <a href="{{ route('keuangan.pembayaran.download-bukti', $pembayaran->id) }}" class="btn btn-primary btn-sm">
...
@push('scripts')
<script>
    const jenisPembayaran = '{{ $pembayaran->jenis_pembayaran }}';
    document.addEventListener('DOMContentLoaded', function() {
        const noteEl = document.getElementById('catatan_verifikasi');
        if (!noteEl) return;

        const messages = {
            terverifikasi: jenisPembayaran == 'akreditasi' ? 'LAMDEPILAR menyetujui Pembayaran akreditasi. Mohon dapat melanjutkan ke tahap selanjutnya yaitu pengisian dan/upload Dokumen akreditasi' : 'LAMDEPILAR menyetujui Pembayaran banding. Mohon memantau tahap selanjutnya yaitu penugasan asesor banding'
        ...
        function applyMessageFromSelectedRadio() {
            ...
            noteEl.value = messages[checked.value] || '';
        }
        ...
        document.querySelectorAll('input[name="status_pembayaran"]').forEach(function(radio) {
            radio.addEventListener('change', function() {
                noteEl.value = messages[this.value] || '';
            });
        });
    });
