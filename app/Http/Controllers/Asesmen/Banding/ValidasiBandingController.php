...
class ValidasiBandingController extends Controller
{
    public function index()
    {
        ...
        $assignments = AsesmenUserRole::with([
            ...
            'asesmen.asesmenUserRoles' => function ($q) {
                $q->whereHas('role', fn($query) => $query->where('name', 'asesor_banding'))
        ...
        $validationProgressMap = PenilaianElemenAkBanding::select(
            'id_asesmen',
            'id_asesor',
        ...
        return view('asesmen.banding.ak-banding.validasi.index', compact(
            'rows',
            'totalRows',
            'stats',
            'asesorsPendingList'
     ...
     */
    public function asesor($idAsesmen, $jenisAsesmen = 'ak_banding')
    {
        $user = Auth::user();
        ...
        // Get AsesmenKecukupanBanding or AsesmenLapanganBanding
        if ($jenisAsesmen === 'ak_banding') {
            $asesmenDetail = AsesmenKecukupanBanding::where('id_asesmen', $idAsesmen)->firstOrFail();
        } else {
            $asesmenDetail = AsesmenLapanganBanding::where('id_asesmen', $idAsesmen)->firstOrFail();
        }
        ...
        if ($asesors->count() < 2) {
            abort(422, 'Minimal 2 asesor banding harus menerima penawaran untuk memulai validasi.');
        }
        ...
        if ($asesorsNeverSubmitted->count() > 0) {
            ...
            abort(422, "Validasi belum dapat dilakukan. Menunggu {$asesorsNeverSubmitted->count()} asesor banding untuk submit penilaian pertama kali: {$asesorNames}");
        }

        // Calculate progress for each asesor
        $totalElemen = ElemenStandar::count();
        // Ambil semua penilaian semua asesor di asesmen ini
        $penilaianAll = PenilaianElemenAkBanding::where('id_asesmen', $idAsesmen)
        ...
        $kriterias = Kriteria::with([
            ...
            'elemenStandar.penilaianElemenAkBanding' => function ($query) use ($idAsesmen, $asesors) {
        ...
        if ($assignment->status_pekerjaan === 'not_started') {
            ...
            if ($pengajuan) {
                ...
                $pengajuan->checkUpdateStatusAKAL('ak_banding', 'status_asesor_on_validation');
                $pengajuan->statusLog()->firstOrCreate(
                    [
                        'status_from' => $statusFrom,
                        'status_to'   => PengajuanAkreditasi::STATUS_AK_BANDING_ON_VALIDATION,
                    ],
                    [
                        'changed_by'  => Auth::id(),
                        'keterangan'  => 'Validator AK telah memulai proses validasi dari penilaian asesor AK Banding',
                        'changed_at'  => now(),
                    ]
                );
            }
        ...
        $typeDoc = $jenisAsesmen === 'ak_banding' ? 'laporan_validasi_ak_banding' : 'laporan_al_banding';
        ...
        return view('asesmen.banding.ak-banding.validasi.asesor', compact(
            'assignment',
            'asesmen',
            'asesmenDetail',
            'jenisAsesmen',
    ...
    public function getValidasiDetail($idAsesmen, $elemenId)
    {
        try {
            ...
            // Get validasi data
            $validasi = PenilaianElemenAkBanding::where('id_asesmen', $idAsesmen)
            ...
            $penilaianAsesor = PenilaianElemenAkBanding::where('id_asesmen', $idAsesmen)
                ->where('id_elemen', $elemenId)
                ->with('asesor')
                ->get();
    ...
    public function getElemenDetail(Request $request, Asesmen $asesmen, ElemenStandar $elemen)
    {
        ...
        $penilaianElemen = PenilaianElemenAkBanding::where('id_asesmen', $asesmen->id)
        ...
        try {
            ...
            if (in_array($request->status, ['validated', 'validated_diff'])) {
                ...
                if (!$request->has('id_asesors') || !is_array($request->id_asesors) || count($request->id_asesors) === 0) {
                    return response()->json([
                        'success' => false,
                        'message' => 'Pilih minimal 1 asesor banding untuk revisi'
                    ], 422);
                }
    ...
    public function validateAllAgreed($idAsesmen)
    {
        ...
        try {
            ...
            foreach ($elemenIds as $elemenId) {
                // Get ALL penilaian for this elemen
                $penilaianList = PenilaianElemenAkBanding::where('id_asesmen', $idAsesmen)
                    ->where('id_elemen', $elemenId)
                    ->whereNotNull('skor')
                    ->get();
    ...
    public function approveAllPenilaian($idAsesmen)
    {
        ...
        try {
            ...
            foreach ($elemenIds as $elemenId) {
                $validatedCount = PenilaianElemenAkBanding::where('id_asesmen', $idAsesmen)
                    ->where('id_elemen', $elemenId)
            ...
            if ($pengajuan) {
                $statusFrom = $pengajuan->status;
                $pengajuan->checkUpdateStatusAKAL('ak_banding', 'status_asesor_selesai');
                $pengajuan->statusLog()->firstOrCreate(
                    [
                        'status_from' => $statusFrom,
                        'status_to'   => PengajuanAkreditasi::STATUS_AK_BANDING_SELESAI,
                    ],
                    [
                        'changed_by'  => Auth::id(),
                        'keterangan'  => 'Validator AK telah menyelesaikan proses validasi dari penilaian asesor AK Banding',
                        'changed_at'  => now(),
                    ]
                );
            }
            ...
            return response()->json([
                'success' => true,
                'message' => 'Semua penilaian banding telah disetujui dan dikunci',
            ]);
        } catch (\Exception $e) {
    ...
    public function exportComparison(Request $request, Asesmen $asesmen)
    {
        ...
        [$tempFile, $filename] = ValidasiExcelService::generateTemplate($asesmen, $asesors, 'ak_banding', $mode);
