...
use App\Models\PenilaianElemenBanding;
...
class BandingController extends Controller
{
    ...
    public function berkas()
    {
        ...
        $asesmens = Asesmen::whereHas('userRoles', function ($query) use ($user) {
            $query->where('id_user', $user->id)->where('jenis_asesmen', 'banding')->where('id_role', Role::ID_ROLE_ASESOR_BANDING);
        })
            ->with([
        ...
        return view('asesmen.banding.berkas.index', compact('asesmens', 'statusPekerjaan'));
    }
    ...
    public function showBerkas($idAsesmen)
    {
        ...
        $this->updateStatusBanding($assignment);
        ...
        $needsRevisions = PenilaianElemenBanding::where('id_asesmen', $asesmen->id)
            ->where('id_asesor', $user->id)
        ...
        // ✅ Ambil semua asesor TIM dulu, SEBELUM updateStatusBanding
        ...
        return view('asesmen.banding.berkas.show', compact(
            'asesmen',
            'kriterias',
            'progress',
            'jenjangs',
    ...
    private function updateStatusBanding($assignment)
    {
        ...
        if ($assignment->status_pekerjaan === 'not_started') {
            ...
            if ($pengajuan) {
                ...
                $pengajuan->checkUpdateStatusAKAL('banding', 'status_asesor_in_progress');
                $pengajuan->statusLog()->firstOrCreate(
                    ...
                    [
                        'changed_by'  => Auth::id(),
                        'keterangan'  => 'Asesor Banding telah memulai proses penilaian surveillance banding',
                        'changed_at'  => now(),
                    ]
                );
            }
    ...
    public function simpanNilai(Request $request, $idAsesmen)
    {
        ...
        try {
            ...
            $penilaian = PenilaianElemenBanding::updateOrCreate(
                [
    ...
    private function calculateProgressBulk($asesmenIds, $userId)
    {
        ...
        $penilaian = PenilaianElemenBanding::where('id_asesor', $userId)
    ...
    public function getHeatmapData($idAsesmen)
    {
        ...
        $heatmapData = PenilaianElemenBanding::where('id_asesmen', $idAsesmen)
            ->where('id_asesor', $user->id)
            ->with([
                'elemen.kriteria'
            ])
    ...
    public function submitPenilaian(Request $request, $idAsesmen)
    {
        try {
            ...
            $assessedElemen = PenilaianElemenBanding::where('id_asesmen', $idAsesmen)
                ->where('id_asesor', $user->id)
                ->whereNotNull('skor')
            ...
            PenilaianElemenBanding::where('id_asesmen', $idAsesmen)
                ->where('id_asesor', $user->id)
                ->update([
                    'status' => 'approved',
                ]);
            ...
            if ($pengajuan) {
                ...
                if ($hasUnapproved) {
                    ...
                    $statusFrom = $pengajuan->status;
                    $pengajuan->checkUpdateStatusAKAL('banding', 'status_asesor_selesai');
                    $pengajuan->statusLog()->firstOrCreate(
                        [
                            'status_from' => $statusFrom,
                            'status_to'   => PengajuanAkreditasi::STATUS_AL_BANDING_SELESAI,
                        ],
                        [
                            'changed_by'  => Auth::id(),
                            'keterangan'  => 'Seluruh asesor Banding telah menyelesaikan proses penilaian surveillance banding',
                            'changed_at'  => now(),
                        ]
                    );
                }
    ...
    public function unsubmitPenilaian($idAsesmen)
    {
        try {
            ...
            $hasValidated = PenilaianElemenBanding::where('id_asesmen', $idAsesmen)
     ...
     * ============================================
     *
     * ADD THIS METHOD TO: App\Http\Controllers\BandingController
    ...
    public function resetAllPenilaian($idAsesmen)
    {
        try {
            ...
            try {
                ...
                $totalDeleted = PenilaianElemenBanding::where('id_asesmen', $idAsesmen)
                    ->where('id_asesor', $user->id)
                    ->count();
    ...
    public function downloadTemplate($idAsesmen)
    {
        try {
            ...
            $asesors = $asesmen->userRoles->filter(function ($userRole) {
                return $userRole->id_role === Role::ID_ROLE_ASESOR_BANDING; // atau role_id == 2
            })->values(); // Reset keys
            ...
            $excelService = new PenilaianExcelService(PenilaianElemenBanding::class);
    ...
    public function exportExcel(Request $request, $idAsesmen)
    {
        try {
            ...
            $excelService = new PenilaianExcelService(PenilaianElemenBanding::class, $mode, $useColor); // atau PenilaianElemenBanding
    ...
    public function getComparisonData(Asesmen $asesmen)
    {
        try {
            ...
            // Get all asesors for this asesmen (Banding)
    ...
    public function exportLaporanPdf($idAsesmen)
    {
        ...
        // ambil daftar asesor Banding accepted (untuk cover)
        $asesors = AsesmenUserRole::where('id_asesmen', $idAsesmen)
            ->where('jenis_asesmen', 'banding')
            ->where('status_penawaran', 'accepted')
            ->whereHas('role', fn($q) => $q->where('name', 'asesor'))
        ...
        $rows = ElemenStandar::with('kriteria')
            ...
            ->map(function ($elemen) use ($idAsesmen, $user) {
                $p = PenilaianElemenBanding::where('id_asesmen', $idAsesmen)
                    ->where('id_asesor', $user->id)
        ...
        $mainPdfPath = $tmpDir . '/laporan_banding_main_' . $asesmen->code . '_' . $user->id . '.pdf';
    ...
    <div style="text-align:center;">
        ...
        <div style="font-size:12px; margin-top:4px;">(Banding)</div>
    ...
    <div style="margin-top:18px; font-size:10px; color:#555;">
        Dokumen ini dihasilkan oleh sistem dan merupakan bagian dari proses surveillance banding.
    </div>
    ...
    ';
        ...
        $pdf->writeHTML('<h3 style="margin:0;">Rekap Penilaian Elemen (Banding)</h3>
    ...
    public function uploadExcelPage($idAsesmen)
    {
        ...
        // ✅ Calculate progress untuk Banding
        $progress = $this->calculateProgressBulk([$asesmen->id], $user->id)[$asesmen->id];
        ...
        return view('asesmen.banding.berkas.upload-excel', compact(
            'asesmen',
            'assignment',
            'progress',
            'statusPekerjaan',
    ...
    public function importExcel(Request $request, $idAsesmen)
    {
        ...
        try {
            ...
            // Dispatch job
            ImportPenilaianExcelJob::dispatch(PenilaianElemenBanding::class, $filePath, $asesmen->id, $user->id, $importLog->id);
