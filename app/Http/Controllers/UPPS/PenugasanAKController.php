<?php
// app/Http/Controllers/UPPS/PenugasanAKController.php
...
{
    /**
     * Display list of penugasan asesor AK
     */
    public function index(Request $request)
    {
        ...
        return view('upps.penugasan-ak.index', compact(
            'pengajuans',
            'stats',
            'tahunList'
        ));
    ...
    /**
     * Show detail penugasan asesor AK
     */
    public function show($id)
    {
        $pengajuan = PengajuanAkreditasi::with([
            ...
            'pengaju',
            'asesmen.asesorAK',
            'asesmen.validatorAK', // ✅ Load validator AK
            'dokumen' => fn($q) => $q->whereIn('jenis_dokumen', [
                'draft_borang',
                'borang_final',
                'surat_tugas_asesor_ak',      // ✅ Load surat tugas asesor
                'surat_tugas_validator_ak'    // ✅ Load surat tugas validator
        ...
        return view('upps.penugasan-ak.show', compact('pengajuan'));
    }
    ...
    private function calculateStatistics($studyProgramIds): array
    {
        ...
        foreach ($logsByPengajuan as $pengajuanId => $pengajuanLogs) {
            ...
            // Terlaporkan: sudah dilaporkan validasi
            if (in_array(PengajuanAkreditasi::STATUS_VALIDASI_BORANG_DILAPORKAN, $statuses)) {
                $stats['terlaporkan']++;
            }

            // Asesor ditugaskan: sudah ada penugasan asesor AK
            if (
                in_array(PengajuanAkreditasi::STATUS_ASESOR_AK_ASSIGNED, $statuses) ||
                in_array(PengajuanAkreditasi::STATUS_AK_IN_PROGRESS, $statuses) ||
                in_array(PengajuanAkreditasi::STATUS_AK_ON_VALIDATION, $statuses) ||
                in_array(PengajuanAkreditasi::STATUS_AK_SELESAI, $statuses) ||
                in_array(PengajuanAkreditasi::STATUS_AK_DILAPORKAN, $statuses)
            ) {
