...
class PelaksanaanALBandingController extends Controller
{
    ...
    public function index(Request $request)
    {
                ...
                case 'perlu_validator':
                    ...
                    $query->whereExists(function ($q) {
                        $q->select(DB::raw(1))
                            ->from('pengajuan_status_log as l')
                            ->whereColumn('l.id_pengajuan', 'pengajuan_akreditasi.id')
                            ->where('l.status_to', PengajuanAkreditasi::STATUS_AL_BANDING_SELESAI);
        ...
        if ($request->ajax() || $request->wantsJson()) {
            $html = view('de.banding.pelaksanaan-al-banding.components.table-content', compact('pengajuans'))->render();
        ...
        return view('de.banding.pelaksanaan-al-banding.index', compact(
            'pengajuans',
            'stats',
            'universities',
            'availableValidators'
    ...
    public function show($id)
    {
        $pengajuan = PengajuanAkreditasi::with([
            ...
            'asesmen.beritaAcaraAL' => function ($q) {
                $q->where('type', 'berita_acara_al_banding')
        ...
        $validatorAK = $pengajuan->asesmen?->asesmenKecukupanBanding?->validators()
    ...
    public function assignValidator(Request $request, $id)
    {
        ...
        try {
            ...
            $asesmenLapanganBanding = $asesmen->asesmenLapanganBanding;
            ...
            if ($useValidatorAK) {
                // Get validator AK
                $validatorAK = $asesmen->asesmenKecukupanBanding?->validators()
                    ->whereIn('status_penawaran', ['accepted', 'pending'])
                    ->first();
            ...
            if ($exists) {
                return response()->json([
                    'success' => false,
                    'message' => 'Validator telah ditugaskan untuk pelaporan AL banding ini'
                ], 422);
            }
            ...
            $message = "Validator {$user->name} berhasil ditugaskan untuk rekap berita acara & pelaporan AL banding.";
