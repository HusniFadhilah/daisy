...
class ValidasiAKBandingController extends Controller
{
    /**
    ...
    public function index(Request $request)
    {
        ...
        $query = PengajuanAkreditasi::with([
            ...
            ->whereHas('asesmen.asesmenKecukupanBanding'); // Must have AK
        ...
        if ($request->filled('status_validasi')) {
            $map = [
                'belum_mulai'      => PengajuanAkreditasi::STATUS_ASESOR_AK_BANDING_ASSIGNED,
                'sedang_penilaian' => PengajuanAkreditasi::STATUS_AK_BANDING_IN_PROGRESS,
                'sedang_validasi'  => PengajuanAkreditasi::STATUS_AK_BANDING_ON_VALIDATION,
                'selesai'          => PengajuanAkreditasi::STATUS_AK_BANDING_SELESAI,
                'dilaporkan'       => PengajuanAkreditasi::STATUS_AK_BANDING_DILAPORKAN,
            ];
        ...
        if ($request->ajax() || $request->wantsJson()) {
            $html = view('de.banding.validasi-ak-banding.components.table-content', compact('pengajuans'))->render();
        ...
        return view('de.banding.validasi-ak-banding.index', compact('pengajuans', 'stats', 'universities'));
    }
    ...
    public function show($id)
    {
        ...
        if ($pengajuan->asesmen) {
            foreach ($pengajuan->asesmen->asesmenUserRoles as $assignment) {
                ...
                } else if ($roleName === 'asesor_banding') {
                    $completed = $asesorProgress[$userId]->completed ?? 0;
                    $pending = $totalElements - $completed;
        ...
        $fullyValidatedElements = DB::table('penilaian_elemen_ak_banding')
            ->where('id_asesmen', $pengajuan->asesmen->id ?? 0)
            ->groupBy('id_elemen')
        ...
        return view('de.banding.validasi-ak-banding.show', compact(
            'pengajuan',
            'userProgress',
            'totalElements',
            'validationSummary'
    ...
    private function calculateStatistics()
    {
        ...
        $belumMulai = (clone $base)
            ->where('status', PengajuanAkreditasi::STATUS_ASESOR_AK_BANDING_ASSIGNED)
            ->count();

        $sedangPenilaian = (clone $base)
            ->where('status', PengajuanAkreditasi::STATUS_AK_BANDING_IN_PROGRESS)
            ->count();

        $sedangValidasi = (clone $base)
            ->where('status', PengajuanAkreditasi::STATUS_AK_BANDING_ON_VALIDATION)
            ->count();

        // tetap pakai status log (historical)
        $selesai = (clone $base)->whereHas(
            'statusLog',
            fn($q) => $q->where('status_to', PengajuanAkreditasi::STATUS_AK_BANDING_SELESAI)
        )->count();

        $dilaporkan = (clone $base)->whereHas(
            'statusLog',
            fn($q) => $q->where('status_to', PengajuanAkreditasi::STATUS_AK_BANDING_DILAPORKAN)
        )->count();
    ...
    public function getValidationTimeline($id)
    {
        try {
            ...
            if (!$pengajuan->asesmen) {
                return response()->json([
                    'success' => false,
                    'message' => 'Asesmen banding not found'
                ], 404);
            }

            // Get penilaian dengan validasi info
            $penilaianTimeline = DB::table('penilaian_elemen_ak_banding as p')
